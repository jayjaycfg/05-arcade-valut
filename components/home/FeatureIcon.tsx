...
// Pixel-style 16x16 SVG icons drawn from rects; color comes from the parent's
// `currentColor` so each feature card can recolor its own icon.
export function FeatureIcon({ kind }: { kind: FeatureIconKind }) {
	const C = 'currentColor';
	if (kind === 'GAMEPAD')
		return (
			<svg aria-hidden="true" className="ft-icon" viewBox="0 0 16 16">
				<g fill={C}>
